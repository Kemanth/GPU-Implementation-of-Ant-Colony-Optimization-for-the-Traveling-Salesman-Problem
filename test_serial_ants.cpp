...
//compare this part with the previous serial ants, i think this will be faster and easier to implement in parallel 
void  calcFitness()
{	
	for(int i = 0; i < MAX_CITIES; i++){
		for(int j = 0; j<MAX_CITIES; j++){
			fitness[i][j] = pow( pheromone[i][j], ALPHA) * pow( (1.0/ dist[i][j]), BETA);
		}
	}
}
